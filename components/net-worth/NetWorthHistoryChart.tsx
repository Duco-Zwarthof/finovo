...
export default function NetWorthHistoryChart({
  ...
  const data: ChartPoint[] = snapshots.map(
    (snapshot) => ({
      date: snapshot.date,
    ...
    <section className="rounded-3xl border border-white/10 bg-zinc-900 p-6">
      <div>
        ...
        <p className="mt-1 text-sm leading-6 text-zinc-500">
          Daily snapshots of the accounts included in your net worth.
        </p>
      </div>
        ...
        <div className="mt-8 flex min-h-72 items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-6 text-center">
          <div>
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-zinc-500">
              The graph will appear after Finovo has saved snapshots on at least two different days.
            </p>
          </div>
        </div>
      ) : (
